...
class epub_paginator:
    ...
    **Version 2.95**
    ...
    looking for a </span> apler the epub:type="pagebreak" element.
    ...
    **Version 2.0**
    ...
    def __init__(self):
        ...
        **_DEBUG_**
        ...
        """
        self.outdir = (
            "/Users/tbrown/Documents/projects/" "BookTally/paged_epubs"
        )
        self.match = True
        self.genplist = True
        self.pgwords = 300
        ...
        **_bk_flist_** -- the list of files to zip into the epub file
        ...
        with zipfile.ZipFile(epub_path, "w") as myzip:
            if "mimetype" in bk_flist:
                ...
                self.wrlog(True, "Fatal error, no mimetype file was found.")
                if self.DEBUG:
        ...
        with zipfile.ZipFile(epub_path, "a", zipfile.ZIP_DEFLATED) as myzip:
            for ifile in bk_flist:
                myzip.write(
                    f"{srcfiles_path}/{ifile}", ifile, zipfile.ZIP_DEFLATED
                )
        ...
        if self.bkinfo["version"][0] != "3":
            ...
            if self.ebookconvert.casefold() != "none":
                ...
                if result.returncode == 0:
                    ...
                    plstr = (
                        f'<nav epub:type="page-list" id="page-list" '
                        'hidden="hidden"><ol>'
                    ) + CR
                    self.plist = plstr
        ...
        if loc == -1:
            ...
            sys.exit(
                (
                    ...
                    "apler having been initially found."
                )
            )
    ...
    def bld_foot(self, curpg, sct_pg, sct_pgcnt):
        ...
        **_sct_pgcnt_**
        ...
        if self.pl_color == "none":
            if self.pl_align == "float":
                # the following creates center justified bookpages and right
                # justified chapter pages.
                pageline = (
                    f'<div> <p style="font-size:75%; '
                    f'float: lepl;margin: 0 0 0 0">{pagestr_bookpages}'
                ...
                pageline = (
                    f'<div style="font-size:{self.pl_fntsz}; '
    ...
    def bld_href(self, manifest_item):
        ...
        **_manifest_item_**
        ...
        if nn == nav_path:
            ...
            if self.DEBUG:
                lstr = (
                    f"Oops, opf_path: {opf_path}; manifest_path: "
                    f"{manifest_path}"
                )
                self.wrlog(True, lstr)
            href = urllib.parse.quote(manifest_item["@href"])
        ...
        with opf_filep.open("r") as opf_file:
            ...
            if self.genplist:
                ...
                if self.bkinfo["nav_file"] == "None":
                    self.wrlog(
                        True, ("Fatal error - did not find navigation file")
                    )
                    self.rdict["fatal"] = True
                    return
                else:
            ...
            for m_item in opf_dict[opf_dictkey][manifest_key]["item"]:  # type: ignore
                if spine_item["@idref"] == m_item["@id"]:
                    if (
                        ...
                        fdict["disk_file"] = f"{disk_path}{uqdfile}"
    ...
    def process_html(self, ebdata):
        ...
        if eltype == "/body":
            # self.wrlog(True, f"done: html element: {stat['el']}")
            stat["done"] = True
        ...
        **_chapter_**
        ...
        while not done:
            ...
            if ebook_data[0] == "<":
                ...
                if stat["done"]:
                    # self.wrlog(True,(f"writing last: "
                ...
                if self.pg_wcnt > self.pgwords:
                    ...
                    # self.wrlog(True, (f"Page boundary. pg_wcnt: "
                    ...
                    # one line.
                    if need < 10:
                        # self.wrlog(True, f"Placing plist beginning")
                        if self.genplist:
                            pstr = (
            ...
            if plink_loc == -1 and plink_loc1 == -1:
                ...
                if loctitle == -1 and loclabel == -1 and locid == -1:
                    ...
                    if loc == -1:
                        ...
                        lstr = (
                            "Did not find paragraph end to "
                            "insert matched pageline."
                        )
                        self.wrlog(True, lstr)
                        lstr = (
                            f"--> thispage: {thispage}; " f"sct_pg: {sct_pg}"
                        )
                        self.wrlog(True, lstr)
                    else:
                        loc += 4
        ...
        **_source_epub_** -- The original ePub file to modify.
        ...
        if self.bkinfo["has_plist"] and self.bkinfo["match"]:
            ...
            self.wrlog(
                True,
                (
                    f"Approximately {self.bkinfo['pgwords']}"
                    f" words per page."
                ),
            )
        elif self.genplist:
        ...
        self.ePubZip(
            (f"{self.rdict['bk_outfile']}"),
            (f"{self.outdir}/" f"{self.bkinfo['title']}"),
            self.bk_flist,
        )
